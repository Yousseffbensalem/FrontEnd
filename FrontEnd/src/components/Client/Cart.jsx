...
const { cartDetails, removeItem , clearCart,
cartCount,incrementItem,decrementItem } = useShoppingCart();
const totalPrice = cartDetails 
    ? Object.values(cartDetails).reduce((total, item) => total + item.prix * item.quantity, 0)
    : 0;
...
<img src={`${cartItem.imageart}`} alt={cartItem.title}/>
<div>
<h3>{cartItem.designation}</h3>
<button onClick={() => removeItem(cartItem.id)}>
<i className="fa-solid fa-trash-can"style={{"fontSize":"10px","color":"red"}}></i>
...
{cartItem.quantity*cartItem.prix} TND
</div>
</div>
)}
</div>
